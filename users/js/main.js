...
/******/ (() => { // webpackBootstrap
    ...
    const iconDisplay = document.querySelector('.icon_temperature');
    ...
    const iconRainDisplay = document.querySelector('.icon_rain');
    const avghumidityDisplay = document.querySelector('.avghumidity');
    const rainconditionDisplay = document.querySelector('.raincondition');
    ...
    async function fetchData(location) {
        try {
            ...
            //   const response = await fetch(`https://api.weatherapi.com/v1/current.json&forecast.json?key=cd77a577f4344b949c0205340231505&q=${location}&days=3`, requestOptions)
    ...
    function Data(data) {
        ...
        this.icon_rain = data.forecast.forecastday[0].day.condition.icon;
        this.raincondition = data.forecast.forecastday[0].day.condition.text;
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const forecastText = dataList[i].day.condition.text;
            const avgtemp_c = Math.round(dataList[i].day.avgtemp_c);
        ...
        for (let i = 0; i < 24; i++) {
            ...
            const hourlyForecastText = hourlyDataList[i].condition.text;
        ...
        for (let i = 0; i < 24; i++) {
            ...
            const nextDayForecastText = nextDayData[i].condition.text;
    ...
    function displayData(data) {
        ...
        iconRainDisplay.src=data.icon_rain;
        avghumidityDisplay.textContent = data.humidity;
        rainconditionDisplay.textContent = data.raincondition;
        ...
        // if (forecastType === 'daily') {
            for (let i = 0; i < 3; i++) {
                ...
                if (unitType === 'far') {
                    ...
                    avgTemp = data.dailyForecastArray[i].avgtemp_c;
                }
